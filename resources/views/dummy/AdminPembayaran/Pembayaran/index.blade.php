@extends('AdminPembayaran.component')
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="text" name="search" class="form-control"
                                            placeholder="ID, Nama User, atau Fasilitas" value="{{ $search ?? '' }}">
                    ...
                    <table class="table table-hover text-nowrap">
                        ...
                        <tbody>
                            @forelse($checkouts as $checkout)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <div>
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <span>
                                                    <strong>Rp
                                                        {{ number_format($checkout->total_dibayar, 0, ',', '.') }}</strong>
                                                    /
                                                    <span class="text-muted">Rp
                                                        {{ number_format($checkout->total_bayar, 0, ',', '.') }}</span>
                                                </span>
                                            </div>
                                            <div class="progress mt-1" style="height: 7px;">
                                                <div class="progress-bar bg-success" role="progressbar"
                                                    style="width: {{ $checkout->persen_dibayar }}%;"
                                                    aria-valuenow="{{ $checkout->persen_dibayar }}" aria-valuemin="0"
                                                    aria-valuemax="100">
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <td>
                                        @if ($checkout->status == 'kompensasi')
                                            ...
                                            @php
                                                $pendingPayment =
                                                    $checkout->pembayaran->where('status', 'pending')->count() > 0;
                                                $rejectedPayment =
                                                    $checkout->pembayaran->where('status', 'ditolak')->count() > 0;
                                                $latestPayment = $checkout->pembayaran
                                            ...
                                            @elseif ($latestPayment && $latestPayment->status == 'ditolak')
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group">
                                            <a href="{{ route('petugas_pembayaran.pembayaran.show', $checkout->id) }}"
                                                class="btn btn-sm btn-info" title="Lihat Detail">
                                                <i class="fas fa-eye"></i>
                                            </a>

                                            @if ($checkout->status == 'kompensasi' && $checkout->pembayaran->where('status', 'pending')->count() > 0)
                                                ...
                                                <button type="button" class="btn btn-sm btn-danger" data-toggle="modal"
                                                    data-target="#tolakModal{{ $checkout->id }}" title="Tolak Pembayaran">
                                ...
                                <!-- Modal Verifikasi -->
                                @if ($checkout->status == 'kompensasi' && $checkout->pembayaran->where('status', 'pending')->count() > 0)
                                    <div class="modal fade" id="verifikasiModal{{ $checkout->id }}" tabindex="-1"
                                        role="dialog" aria-labelledby="verifikasiModalLabel{{ $checkout->id }}"
                                        aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header bg-success">
                                                    <h5 class="modal-title" id="verifikasiModalLabel{{ $checkout->id }}">
                                                        Verifikasi Pembayaran #{{ $checkout->id }}</h5>
                                                ...
                                                <form
                                                    action="{{ route('petugas_pembayaran.pembayaran.verifikasi', $checkout->pembayaran->where('status', 'kompensasi')->first()->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="alert alert-info">
                                                            ...
                                                            <p>Anda akan memverifikasi pelunasan pembayaran untuk booking
                                                                #{{ $checkout->id }}.</p>
                                                            <ul class="mb-0">
                                                                <li>Total: Rp
                                                                    {{ number_format($checkout->total_bayar, 0, ',', '.') }}
                                                                </li>
                                                                <li>Sudah Dibayar: Rp
                                                                    {{ number_format($checkout->total_dibayar, 0, ',', '.') }}
                                                                </li>
                                                                <li>Sisa Pembayaran: Rp
                                                                    {{ number_format($checkout->sisa_pembayaran, 0, ',', '.') }}
                                                                </li>
                                                            </ul>
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="catatan{{ $checkout->id }}">Catatan
                                                                (opsional)
                                                                :</label>
                                                            <textarea class="form-control" id="catatan{{ $checkout->id }}" name="catatan" rows="3"
                                    ...
                                    <!-- Modal Tolak -->
                                    <div class="modal fade" id="tolakModal{{ $checkout->id }}" tabindex="-1"
                                        role="dialog" aria-labelledby="tolakModalLabel{{ $checkout->id }}"
                                        aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header bg-danger">
                                                    <h5 class="modal-title" id="tolakModalLabel{{ $checkout->id }}">Tolak
                                                        Pembayaran #{{ $checkout->id }}</h5>
                                                    <button type="button" class="close" data-dismiss="modal"
                                                        aria-label="Close">
                                                        <span aria-hidden="true">&times;</span>
                                                    </button>
                                                ...
                                                <form
                                                    action="{{ route('petugas_pembayaran.pembayaran.tolak', $checkout->pembayaran->where('status', 'pending')->first()->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <input type="hidden" name="checkout_id"
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="alert alert-warning">
                                                            <h6><i class="icon fas fa-exclamation-triangle"></i> Perhatian!
                                                            </h6>
                                                            <p>Anda akan menolak pembayaran untuk booking
                                                                #{{ $checkout->id }}.</p>
                                                            <p>Status booking akan tetap "DP" dan user harus melakukan
                                                                pembayaran ulang.</p>
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="alasan_tolak{{ $checkout->id }}">Alasan Penolakan:
                                                                <span class="text-danger">*</span></label>
                                                            <textarea class="form-control" id="alasan_tolak{{ $checkout->id }}" name="alasan_tolak" rows="3" required
                                ...
                                <tr>
                                    <td colspan="8" class="text-center py-3">
                                        <div class="alert alert-info mb-0">
                                            <i class="fas fa-info-circle mr-2"></i> Tidak ada data pembayaran yang
                                            ditemukan
                                        </div>
                                    </td>
                                </tr>
